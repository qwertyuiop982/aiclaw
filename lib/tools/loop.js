...
function formatToolResult(name, ok, payload) {
...
async function runAgentLoop(cfg, initialMessages, opts) {
  const maxSteps = Math.max(1, Math.min(20, (opts && opts.maxSteps) || 6));
  ...
  while (step < maxSteps) {
    step++;
    const data = await chatMod.chat(cfg, messages);
    const { content: out, reasoning } = chatMod.extractContent(data);
    ...
    for (const r of results) {
      messages.push(formatToolResult(r.name, r.ok, r.ok ? r.result : { error: r.error }));
    }

    onStep({ step, text: cleaned, calls, results, stop: false });

    if (step >= maxSteps) break; // next iteration would exceed
  }
